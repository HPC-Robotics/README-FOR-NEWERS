...
## 6.5 VLA相关
我们采用自顶向下的方法来拆解技术，
首先VLA比较简单，实际上你只需要看一篇文章：pi0[pi0](https://arxiv.org/html/2410.24164v1) 就已经掌握这个领域的50%了
### 6.5.1连续动作表示的VLA
#### pi0
...
#### train time RTC
参考这个[](https://arxiv.org/pdf/2512.05964)
#### inference time RTC
参考这个[](https://arxiv.org/pdf/2506.07339)
...
### 6.5.6关于历史
前面说了，现在VLA基本都没有历史，关于历史的工作也有很多，[](https://www.pi.website/download/Mem.pdf)可以参考这篇MEM论文，
显然我们做历史肯定不能直接把所有历史信息全部发给VLM，而是尽可能先融合、再放进vlm，因为图像token位本身就是高度冗余的，比如一个224 224的图就包含了256个token，显然关键的信息并不会用到这么多。
